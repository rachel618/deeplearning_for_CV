...
def hello():
  ...
  """
  print('Hello from pytorch101.py! jihyun')
...
def multiples_of_ten(start, stop):
  ...
  #                    TODO: Implement this function                          #
  #############################################################################
  # Replace "pass" statement with your codex.
  x=torch.arange(start,stop+1)
  bool_idx=(x%10==0)
  if len(x)!=0:
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  idx0=torch.arange(x.shape[0]) #idx0=[0,1,2,...,M-1]
  idx1=torch.arange(x.shape[1]) # idx1=[0,1,2,...,N-1]
  y=torch.zeros(x.shape[0],x.shape[1], dtype=torch.int64)
  for idx in range(x.shape[0]):
      y[idx,:]=x[x.shape[0]-1-idx,:]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
